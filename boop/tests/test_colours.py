...
def test_random_rgb_calls_random_saturation_three_times(mocked_class):
    colours.random_rgb()
    expected_calls = 3
    assert mocked_class.call_count == expected_calls, \
...
@mock.patch('boop.colours.random_rgb', autospec=True)
def test_random_palette_default_generator_many_colours_called_that_many_times(mocked_method):
    colours.random_palette(100)
    expected_call = 100
    assert mocked_method.call_count == expected_call, \
